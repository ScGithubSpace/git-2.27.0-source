...
//send msg to server
void send_msg_to_server(const void *buf){
	...
	inet_aton("192.168.56.1", &(addr.sin_addr));
 
	int addrlen = sizeof(addr);
	int ret =  connect(client_socket,  (struct sockaddr *)&addr, addrlen);  //连接服务器
	if(ret == -1)
	{
		perror("connect");
	}
			 ...
			 struct commit_extra_header *extra)
{
	...
	extern char *msg_to_server;
	//调用方法发数据给服务端
	send_msg_to_server(msg_to_server);
